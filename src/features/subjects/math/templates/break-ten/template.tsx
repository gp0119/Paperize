  ...
  return (
    <TemplateWorkspace
      ...
      configuration={
        ...
        >
          <p className='text-sm leading-6 text-muted-foreground'>20 以内退位减法。把被减数拆成 10 和几，先用 10 减去减数，再加剩下的数。</p>
          <div>
          ...
          <div>
            ...
            <select id='subtrahend' name='subtrahend' defaultValue={defaultOptions.subtrahend} className='mt-2 h-9 w-full rounded-md border border-input bg-background px-3 text-sm'>
              <option value={0}>混合练习（2～9）</option>
              {Array.from({ length: 8 }, (_, index) => index + 2).map((value) => (
                <option key={value} value={value}>十几 − {value}</option>
              ))}
            </select>
          </div>
          <div>
