...
export function TemplateCard({ subject, template, preview }: { subject: SubjectId; template: TemplateDefinition; preview: ReactNode }) {
  return (
    <Link
      ...
      <div className='px-3 pt-2 pb-3'>
        ...
        <div className='mt-2 flex flex-wrap items-center gap-1.5 text-xs text-slate-500'>
          {template.tags?.map((tag) => (
            <span key={tag} className='rounded-md bg-slate-100 px-2 py-1 leading-none'>
              {tag}
            </span>
          ))}
        </div>
